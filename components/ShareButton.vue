  ...
  <div class="relative inline-block">
    ...
    <transition name="fade">
      ...
      >
        ...
        <div v-if="filteredNetworks.length === 0" class="text-center py-2 text-gray-500">
          Aucun réseau trouvé
        </div>
        
        <div v-else class="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-2">
          <!-- Copier le lien -->
          <button 
            @click="copyLink" 
          ...
          >
            <span class="ml-0.5 flex items-center justify-center">
              <NuxtImg 
                ...
                class="w-6 h-6"
              />
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
const props = defineProps({
  ...
  description: {
    type: String,
    default: 'Je viens de découvrir ce tableau et je voulais le partager avec vous.',
  },
  position: {
    type: String,
...
const shareMenuPosition = computed(() => {
  switch(props.position) {
    case 'bottom-left': return 'left-0';
...
const handleClickOutside = (event) => {
  ...
  if (isOpen.value && !event.composedPath().includes(document.querySelector('.relative.inline-block'))) {
    isOpen.value = false;
  }
};
