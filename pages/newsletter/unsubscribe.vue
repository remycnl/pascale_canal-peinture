...
setPageTitle("Désabonnement de la newsletter | Pascale Canal");
...
onMounted(async () => {
	...
	try {
		...
		error.value =
			"Une erreur est survenue lors de la vérification du lien de désabonnement.";
	} finally {
		isLoading.value = false;
	}
});
...
const resubscribe = async () => {
	...
	try {
		...
		if (response.success) {
			...
			error.value =
				response.message || "Échec de la réactivation de l'abonnement.";
			unsubscribeSuccess.value = false;
		}
	} catch (err) {
...
<template>
	<div class="unsubscribe-page py-16 max-w-2xl mx-auto px-4">
		...
		<div v-if="isLoading" class="text-center py-8">
			<div
				class="inline-block w-8 h-8 border-2 border-t-yellow rounded-full animate-spin"></div>
		...
		<div
			...
			<p class="text-gray-600 mb-4">
				Le lien de désabonnement semble invalide ou a expiré.
			</p>
			<nuxt-link
				to="/"
				class="inline-block px-4 py-2 border border-black bg-yellow rounded hover:bg-yellow/70">
				Retour à l'accueil
			</nuxt-link>
		</div>
		...
		<div
			...
			<h2 class="text-xl font-apercuBold mb-2 text-green-700">
		...
		<div v-else class="bg-white border rounded-lg p-6 shadow-sm">
			<div class="mb-6">
				...
				<p class="text-gray-600">
					...
					<span v-if="subscriberEmail">
						L'adresse email concernée est :
			...
			<div class="mb-6" v-if="subscriptionTypes.length > 0">
				...
				<ul class="list-disc pl-5 text-gray-600">
					<li v-for="(type, index) in subscriptionTypes" :key="index">
						{{ type }}
					</li>
				</ul>
			</div>
